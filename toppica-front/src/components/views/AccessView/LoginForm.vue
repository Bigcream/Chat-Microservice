...
import { IAuthUserInterface } from "@src/interface/auth";
const authUserStore = useAuthUserStore()
const showPassword = ref(false);

const payload = reactive<IAuthUserInterface>({
  username: "",
  email: "",
  password: "",
  passwordConfirm: "",
...
const message = computed(() => authUserStore.getMessage)
  ...
  <div class="p-5 md:basis-1/2 xs:basis-full flex flex-col justify-center items-center">
    <div class="w-full md:px-[26%] xs:px-[10%]">
      ...
      <div class="mb-6 flex flex-col">
        <img src="@src/assets/vectors/logo-gradient.svg" class="w-[1.375rem] h-[1.125rem] mb-4 opacity-70"
          alt="bird logo" />
      ...
      <div style=" padding: inherit; padding-bottom: 7px; color: red;">{{ message }}</div>
      <!--form-->
      <form @submit.prevent="submit">
        <div class="mb-6">
          <TextInput label="Email" @value-changed="receiveEmail" placeholder="Enter your email" class="mb-5" />
          <TextInput label="Password" @value-changed="receivePassword" placeholder="Enter your password"
            :type="showPassword ? 'text' : 'password'" class="pr-[2.5rem]">
            <template v-slot:endAdornment>
              <IconButton title="toggle password visibility" aria-label="toggle password visibility" class="m-[.5rem] p-2"
        ...
        <!--local controls-->
        <div class="mb-6">
          <Button type="submit" class="w-full mb-4">Sign in</Button>
        </div>
      </form>
      ...
      <div>
        <Button variant="outlined" class="w-full mb-5">
          <span class="flex">
            <img src="@src/assets/vectors/google-logo.svg" class="mr-3" alt="google logo" />
            Sign in with google
          </span>
        </Button>
