...
import { IAccountValues } from "@src/interface/userdata";
...
const payload = reactive<IAccountValues>({
  id: 0,
  email: "",
  firstName: "",
  lastName: "message",
...
const handleSubmit = () => {
  ...
  payload.avatar = currentuser.value.avatar;
  payload.email = currentuser.value.email;
  payload.firstName = accountValues.value.firstName;
  payload.lastName = accountValues.value.lastName;
  userStore.updateUserInfo(payload, accountValues.value.avatar)
