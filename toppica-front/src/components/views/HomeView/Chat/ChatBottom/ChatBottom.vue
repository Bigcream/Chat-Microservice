...
import { IMessageRequest, IRoomChat } from "@src/interface/chat";
...
const roomChats: Ref<IRoomChat[]> = computed(() => chatStore.listRoomChatByEmail)
...
const handleSetDraft = () => {
  const index = roomChats.value.findIndex(room => room.id == activeConversation.value.id);
  roomChats.value[index].draftMessage = value.value;
};

const handleRemoveDraft = () => {
  const index = roomChats.value.findIndex(room => room.id == activeConversation.value.id);
  roomChats.value[index].draftMessage = "";
};


const payload = reactive<IMessageRequest>({
  id: 0,
  type: "",
  content: "",
  receiverId: 0,
...
const onClick = () => {
  payload.avatar = activeConversation.value.avatar;
  payload.content = value.value;
  payload.type = activeConversation.value.type
  payload.contactId = currentContact.value.id;
  payload.roomChatId = activeConversation.value.id;
  if (activeConversation.value.type === "group") {
    payload.groupName = activeConversation.value.name;
    activeConversation.value.contacts.forEach(function (contact) {
      memberIds.push(contact.id);
    });
    memberIds.push(currentContact.value.id);
    payload.memberIds = memberIds;
  } else {
    payload.groupName = getFullName(activeConversation.value.contacts[0]);
    payload.receiverId = payload.contactId === activeConversation.value.contacts[0].id ? activeConversation.value.contacts[1].id : activeConversation.value.contacts[0].id;
    payload.memberIds = [currentContact.value.id, activeConversation.value.contacts[0].id];
  }
  if(activeConversation.value.replyMessage){
    payload.replyTo = activeConversation.value.replyMessage.id;
...
<template>
  <div class="w-full">
    ...
    <div class="relative transition-all duration-200" :class="{ 'pt-[3.75rem]': activeConversation.replyMessage }">
      <ReplyMessage />
    </div>

    <div class="h-auto min-h-[5.25rem] p-5 flex items-end" v-if="store.status !== 'loading'"
      ...
      <div class="grow md:mr-5 xs:mr-4 self-end" v-if="!recording">
        <div class="relative">
          <Textarea v-model="value" @input="handleSetDraft" :value="value"
            class="max-h-[5rem] pr-[3.125rem] resize-none scrollbar-hidden" auto-resize cols="30" rows="1"
      ...
      <div class="min-h-[2.75rem] flex">
        ...
        <!--send message button-->
        <IconButton v-if="!recording" :disabled="checkValue"
          ...
          variant="ghost" title="send message" aria-label="send message" @click="onClick">
